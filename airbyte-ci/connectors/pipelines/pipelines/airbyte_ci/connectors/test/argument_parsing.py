...
def build_extra_params_mapping(
    ...
    def callback(ctx: click.Context, argument: click.core.Argument, raw_extra_params: Tuple[str]) -> Dict[str, STEP_PARAMS]:
        ...
        for param in raw_extra_params:
            ...
            try:
                step_id = SupportedStepId(step_name).value
            except ValueError:
                raise ValueError(f"Invalid step name {step_name}, it must be one of {[step_id.value for step_id in SupportedStepId]}")
